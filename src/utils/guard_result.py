...
class GuardFinding:
    ...
    guard: str                          # 执行 guard 名（L2 聚合 guard 的父名，注册在 GUARD_RUNNERS）
    ...
    source_guard: str = ""              # 子检测身份（聚合 guard 下属的旧子 guard 名，如 anti_ai_guard）
...
def finding(guard: str, severity: Severity, code: str, message: str,
            ...
            confidence: float = 0.65, location: str = "",
            source_guard: str = "") -> GuardFinding:
    ...
    return GuardFinding(guard=guard, severity=severity, code=code,
                        ...
                        location=location, source_guard=source_guard)
