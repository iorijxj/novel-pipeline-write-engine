...
# AI explanation markers — explanatory connectors in narrative (originally in dialogue_naturalness)
AI_EXPLAIN_PATTERNS = [
    (r"也就是说|换言之|换句话说|简而言之|总而言之|总的来看|可以这么说", "AI_EXPLAIN"),
    (r"这意味着|这就意味着|这说明|这表明|这代表了|这揭示了|这暗示着", "AI_EXPLAIN"),
]
...
def check_anti_ai(content: str) -> Tuple[int, list[dict]]:
    ...
    # 9. Check AI Explanation Markers (解释性连接词 — moved from dialogue_naturalness)
    for pattern, code in AI_EXPLAIN_PATTERNS:
        for m in re.finditer(pattern, content):
            snippet = content[max(0, m.start()-10):m.end()+20].replace("\n", " ")
            findings.append({
                "code": code,
                "message": f"AI解释腔: '{m.group()[:40]}'",
                "sentence": snippet.strip()[:120],
                "location": "",
                "evidence_weight": 1.0,
                "confidence": 0.75,
...
def run_anti_ai_check_result(content: str, chapter_no: int = 0) -> dict:
    ...
    ai_explain_count = sum(1 for f in raw_findings
                            if f["code"] == "AI_EXPLAIN")
