-- ProseForge 数据库 schema（单一权威来源）
-- 与 src/db/slot_manager.py 内嵌 _EMBEDDED_SCHEMA_SQL 保持逐字一致。
-- 18 普通表 + 6 FTS5。无 CREATE INDEX。UTF-8（无 BOM）。
...
-- 声纹记忆子系统（原 database/migrations 提供，现并入单一权威 schema）
...
CREATE TABLE IF NOT EXISTS character_voice_profiles (
    ...
    dialect_pack TEXT DEFAULT '',
    register_pack TEXT DEFAULT '',
    meme_pack TEXT DEFAULT '',
    english_pack TEXT DEFAULT '',
    dialect_level INTEGER DEFAULT 0,
    meme_level INTEGER DEFAULT 0,
    english_level INTEGER DEFAULT 0,
    wenyan_level INTEGER DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS character_voice_examples (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    profile_id INTEGER REFERENCES character_voice_profiles(id),
...
-- migration 追踪表（init_db.run_migrations 也会幂等创建，此处预建以保证单跑 schema 即完整）
