...
ProseForge (v0.8.0) 是一个 AI Agent 辅助的长篇小说工程化系统，提供 Hermes / Codex / Claude 三个插件表面，共享同一个 Python 内核。核心思路不是"怎么写"，而是"怎么保证写对"——门禁系统强制执行写作纪律，30+ 守卫检查每一章。
...
v0.8.0 移除了独立的改写模块。`rewrite` 不再是 `nf_pipeline` 的 action——改稿由 `review` action 输出的 findings 驱动，后续由人工或外部 Agent 完成。原则不变：改稿是增量产物，不覆盖原稿。
